...
const POSTS_JSON_PATH = join(process.cwd(), 'content', 'posts.json')

/**
 * Fetches all posts from the Notion database using the official API.
 * Falls back to local content/posts.json if Notion is unavailable.
 *
 * @param {{ includePages: boolean }} - false: posts only / true: include pages
 */
export async function getAllPosts ({ includePages = false }) {
  const posts = await fetchFromNotion(includePages)

  if (posts.length > 0) return posts

  // Fallback to local backup
  return loadFromLocalBackup(includePages)
}

async function fetchFromNotion (includePages) {
  ...
  const data = allPages.map(page => extractProperties(page)).filter(Boolean)
...
function loadFromLocalBackup (includePages) {
  if (!existsSync(POSTS_JSON_PATH)) {
    console.warn('No local backup found at content/posts.json')
    return []
  }

  console.warn('Notion unavailable — loading posts from local backup')
  try {
    ...
    console.error('Failed to load local backup:', error.message)
    return []
  }
}
